-- TownConnect Enhanced Database Schema
...
-- 8. Create user_interests table for better interest management
...
-- 9. Create event_tags table for better tag management
...
-- 10. Update invites table with additional fields
ALTER TABLE invites 
...
-- Event photos - visible to event attendees or public events
CREATE POLICY "Event photos are viewable by event attendees or public events" ON event_photos
    FOR SELECT USING (
        is_visible = true AND (
            event_id IN (
            ...
            event_id IN (
                SELECT event_id FROM invites WHERE "toUserId" = auth.uid() AND status = 'accepted'
            ) OR
            user_id = auth.uid()
        )
    );
...
-- User interests are manageable by the user
CREATE POLICY "Users can manage their own interests" ON user_interests
    FOR ALL USING (auth.uid() = user_id);

-- Event tags are viewable by everyone, manageable by event hosts
CREATE POLICY "Event tags are viewable by everyone" ON event_tags
    FOR SELECT USING (true);

CREATE POLICY "Event hosts can manage event tags" ON event_tags
    FOR ALL USING (
        event_id IN (
